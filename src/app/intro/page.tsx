      ...
      <>
        <div className="intro-row">
          <div className="intro-text flex-1">
            <p>
              연돌현상(Stack Effect)은 고층건물, 굴뚝, 수직관 등에서 발생하는 자연적인 공기 흐름 현상이다.
              이는 실내외 온도차에 따른 공기 밀도 차이로 인해 부력(Buoyancy force)이 발생하면서 형성된다.
              온도가 높은 공기는 밀도가 낮아 상승하려는 성질이 있으며, 반대로 차가운 공기는 하강하려는
              경향이 있다. 이러한 공기 밀도 차이가 수직 방향의 기류를 유도한다.
            </p>
            <p>
              연돌현상은 자연환기와 같은 공기 유동을 유도하는 데 활용될 수 있으나, 예기치 않은 외기 침투(침기)
              나 실내 공기 누출(누기)의 원인이 될 수 있다. 또한, 연기로 가득 찬 공기가 빠르게 수직 통로를 따라
              이동하는 특성상 화재 확산을 가속하는 위험 요소로도 작용할 수 있다.
            </p>
          </div>
          <div
        ...
        <div className="intro-text">
          ...
          <ul>
            <li>건물 내부와 외부의 온도 차이로 인해 압력차가 형성되며, 이는 연돌현상의 직접적인 원동력이 된다.</li>
            <li>온도차가 클수록 공기 밀도차도 커지므로, 연돌현상에 따른 기류 속도와 압력차가 증가한다.</li>
            <li>겨울철에는 외부 공기가 차고 내부는 난방으로 인해 따뜻하므로, 저층부에서 찬 공기가 유입되고 수직 통로를 따라 따뜻한 공기가 상승하여 상층부로 빠져나간다.</li>
            <li>반대로 여름철에는 실내가 상대적으로 시원하고 외부가 더울 경우, 상층부에서 외부 공기가 유입되어 하층부로 하강하는 기류가 형성된다.</li>
          </ul>
          ...
          <ul>
            <li>연돌현상의 강도는 수직 통로의 높이에 비례한다.</li>
            <li>수직 높이가 클수록 공기기둥의 밀도 차이에서 비롯된 부력이 커지며, 이에 따라 압력차도 커진다.</li>
            <li>고층건물에서는 층별 용도구분, 승객동선편의 등을 위해 엘리베이터 샤프트 구성 방식이 다양화되어 있다. 이로 인해 건물 내부의 연돌기류가 복잡해지고, 예측 및 해석이 어려워지는 경향이 있다.</li>
          </ul>
          ...
          <ul>
            <li>중성대는 건물 내외부의 압력이 같아지는 수직 위치를 의미한다.</li>
            <li>연돌에 의해 발생하는 압력차는 중성대로부터의 상대적인 거리에 따라 결정된다.</li>
            <li>중성대는 연돌기류의 방향과 유속 분포를 판단하는 기준점이 된다. 그러나, 건축계획에 따라 건물마다 다른 위치에 존재하며 두 지점 이상 존재하기도 하여 해석이 복잡하다.</li>
          </ul>
  ...
  },
  {
    ...
    content: (
      <>
        <div className="intro-text">
          <p>
            건물은 외부와 완전히 차단되어 있지 않기 때문에, 연돌현상에 의해 실외 공기가 실내로 침입(침기)하거나
            내부 공기가 유출(누기)되는 공기흐름이 발생한다. 또한, 겨울철과 같이 실내외 온도차가 클 경우, 35층
            고층건물 기준으로 최대 130 Pa 이상의 연돌압력이 발생할 수 있다. 이와 같은 압력차를 고려하지 않은 계획은
            에너지 낭비, 쾌적성 저하, 시스템 오작동, 피난 지연 등 다양한 문제를 유발한다.
          </p>
        </div>
        <hr />
        ...
        <div className="intro-text">
          <h5>
            <span>1</span> 에너지 소비 및 HVAC 시스템 영향
          </h5>
          <ul>
            <li>연돌현상은 계절에 따라 의도하지 않은 외기의 유입을 유발하며, 실내 온도 유지를 위한 냉·난방 부하를 증가시킨다. 이로 인해 전체 에너지 소비가 증가한다.</li>
            <li>고층 건물의 경우 층별로 압력 분포가 달라져 공조 및 환기 시스템 제어가 복잡해지며, 구역 간 부하 불균형이 발생한다.</li>
          </ul>
          ...
          <ul>
            <li>연돌현상에 의해 유입된 찬 공기는 창호 주변에서 복사냉감(radiant discomfort)을 유발하고, 이는 재실자의 열쾌적성을 저하시킨다.</li>
            <li>사용자는 이를 보완하기 위해 난방 온도를 과도하게 설정하게 되며, 결과적으로 에너지 사용량이 증가한다.</li>
            <li>또한, 압력차에 의해 형성된 기류는 오염물질, 병원성 미생물, 바이러스의 수송 경로가 된다. 병원, 요양시설 등 민감한 환경에서는 교차오염 및 2차 감염의 위험을 높인다.</li>
            <li>기류가 도어 틈을 통과할 때 발생하는 고주파 소음(whistling, whooshing)은 재실자의 음향 쾌적성에 악영향을 준다. 특히 엘리베이터 도어에서 35 Pa 이상의 압력차가 작용할 경우, 50 dB(A)를 초과하는 소음이 발생하며, 이는 ASHRAE 권장 기준인 NC 40을 상회한다.</li>
          </ul>
          ...
          <ul>
            <li>과도한 연돌압력은 출입문에 열림·닫힘 저항력을 유발하며, 화재 시 피난 경로의 장애 요인이 된다.</li>
            <li>계단실, 세대 현관, 옥상 출입문 등에서 도어가 열리지 않거나 스스로 닫히는 현상이 발생할 수 있다. 이는 피난 지연과 인명 피해로 이어질 수 있다.</li>
            <li>또한, 연돌기류는 연기를 고속으로 수직 확산시키며, 제연 시스템의 성능을 저하시킨다. 이로 인해 방화구획 유지가 어려워지고, 대피 가능 시간이 단축된다.</li>
          </ul>
        </div>
      </>
    ),
